...
def butter_bandpass_filter(data, sample_rate, lowcut, highcut, order=4):
...
def fir_bandpass_filter(audio_data, sample_rate, lowcut, highcut, order=100):
    nyquist_freq = 0.5 * sample_rate
    low = lowcut / nyquist_freq
    high = highcut / nyquist_freq
    fir_filter = firwin(order + 1, [low, high], pass_zero=False, window='hamming')
    filtered_audio = filtfilt(fir_filter, [1.0], audio_data)
...
def load_recordings(data_folder, data, get_frequencies=False):
    ...
    for i in range(num_locations):
        ...
        #recording = fir_bandpass_filter(recording, sample_rate=frequency, lowcut=20, highcut=600, order=100)
        #recording = butter_bandpass_filter(recording, sample_rate=4000, lowcut=20, highcut=600, order=4)
        
        new_length = round(len(recording) * 0.5)
        recording = signal.resample(recording, new_length)
        recordings.append(recording)
        frequencies.append(frequency)
...
def build_dataset_df3(rec_list, patient_list, patient_murmur_one_hot, window_size):
    new_dict = {0: 1, 2: 2, 4: 3, 6: 4, 8: 5}
    rec_type_arr = []
    patient_id_arr = []
    subframes_arr = []
    murmur_class_arr = []
    ...
    for h in range(len(patient_list)):
        for i in range(0, len(rec_list[h]), 2):
            if rec_list[h][i] == 1:
                ...
                array_rec = np.zeros(nb_frames) + new_dict[i]
                array_id = np.zeros(nb_frames) + int(patient_list[h])
                array_mur_class = [patient_murmur_one_hot[h]] * nb_frames

                rec_type_arr.extend(array_rec)
                patient_id_arr.extend(array_id)
                subframes_arr.extend(sub_arr)
                murmur_class_arr.extend(array_mur_class)

    df = pd.DataFrame({
        'patient_id': patient_id_arr,
        'recording_type': rec_type_arr,
        'recording': subframes_arr,
        'murmur': murmur_class_arr
    })
...
def extract_patients(data_folder,verbose=1):    
    # Find data files.
    if verbose >= 1:
